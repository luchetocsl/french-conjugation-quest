...
import AlpineHintsSimple from "./AlpineHintsSimple";
...
const FrenchGame = ({ challenge }: FrenchGameProps) => {
	...
	const showHint = (index: number) => {
		...
		const hintText = correctForm.substring(
			0,
			Math.ceil(correctForm.length / 2)
		);
	...
	const checkAnswers = () => {
		...
		toast({
			...
			description:
				score === challenge.verbs.length
					? "Parfait ! Excellent travail !"
					: "Continue à pratiquer !",
			variant:
				score === challenge.verbs.length ? "default" : "destructive",
		});
	};
	...
	return (
		<Card className='w-full max-w-2xl mx-auto'>
			...
			<CardContent className='space-y-6'>
				...
				<AlpineHintsSimple hints={generateHints()} />
